    ...
    _memory(VK_NULL_HANDLE), _size(size), _usage(usage), _properties(properties)
{
    ...
    ZE_CHECK_MSG(result == VkResult::VK_SUCCESS, "Create Buffer failed!");
    ...
    ZE_CHECK_MSG(result == VkResult::VK_SUCCESS, "Allocate memory failed!");

    result = vkBindBufferMemory(_device->GetRawDevice(), _buffer, _memory, 0);
    ZE_CHECK_MSG(result == VkResult::VK_SUCCESS, "Bind memory failed!");
}

VulkanBuffer::~VulkanBuffer()
{
    ZE_CHECK(_buffer != VK_NULL_HANDLE);
    vkDestroyBuffer(_device->GetRawDevice(), _buffer, nullptr);

    ZE_CHECK(_memory != VK_NULL_HANDLE);
